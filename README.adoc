...
This module uses the https://registry.terraform.io/modules/terraform-azurerm/aks/azurerm/latest[Terraform module "aks" by Azure] to in order to deploy and manage an AKS cluster. This module was created in order to also manage other resources required by the DevOps Stack, such as the DNS records, resource group and subnet specific to the cluster created (these resources are helpful for the blue/green upgrading strategy). The module also provides the necessary outputs to be used by the other DevOps Stack modules.
